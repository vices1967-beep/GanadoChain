...
contract AnimalNFTUpgradeable is Initializable, ERC721URIStorageUpgradeable, AccessControlEnumerableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    function updateAnimalURI(uint256 tokenId, string calldata newURI) external whenNotPaused onlyRole(PRODUCER_ROLE) {
        require(_exists(tokenId), "No existe token");
        _setTokenURI(tokenId, newURI);
    }

    function burnAnimal(uint256 tokenId) external whenNotPaused onlyRole(PRODUCER_ROLE) {
        require(_exists(tokenId), "No existe token");
        _burn(tokenId);
